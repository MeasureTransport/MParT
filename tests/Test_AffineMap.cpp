...
TEST_CASE( "Testing Rectangular AffineMap", "[RectangularLinearMap]" ) {
    ...
    */
    ...
    for(unsigned int i=0; i<numPts; ++i){
        ...
        CHECK_THAT(evals(0,i), Matchers::WithinAbs(trueOut1, 1e-15));
        CHECK_THAT(evals(1,i), Matchers::WithinAbs(trueOut2, 1e-15));
        CHECK_THAT(pts2(0,i), Matchers::WithinAbs(pts(1,i), 1e-15));
        CHECK_THAT(pts2(1,i), Matchers::WithinAbs(pts(2,i), 1e-15));
    }
