...
DebugMap<MemorySpace>::DebugMap(std::shared_ptr<ConditionalMapBase<MemorySpace>> const& component) : ConditionalMapBase<MemorySpace>(component->inputDim, component->outputDim, component->numCoeffs),
                        comp_(component)
{
...
template<typename MemorySpace>
void DebugMap<MemorySpace>::InverseInplace(StridedMatrix<double, MemorySpace> x,
                                                StridedMatrix<const double, MemorySpace> const& r)
{
    // // Evaluate the output for each component
