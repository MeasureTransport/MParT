...
Below is a comparison of the MParT linear algebra tools and `Eigen <https://eigen.tuxfamily.org/index.php?title=Main_Page>`_.  These examples focus on views defined in the :code:`Kokkos::HostSpace` memory space, but these operators are defined for views in device memory (e.g., :code:`mpart::DeviceSpace`) as well.
...
.. grid:: 2
    ...
    .. grid-item-card::  Matrix Product (Eigen)
    ...
    .. grid-item-card::  Matrix Product (Kokkos+MParT)
...
.. grid:: 2
    ...
    .. grid-item-card::  Transposed Matrix Product (Eigen)
    ...
    .. grid-item-card::  Transposed Matrix Product (Kokkos+MParT)
...
MParT provides a class similar to Eigen's :code:`PartialPivLU` class for LU factorizations.  The :code:`Kokkos::HostSpace` version of MParT's implementation is actually just a thin wrapper around the Eigen implementation.   The :code:`DeviceSpace` version uses the `cuSolver <https://docs.nvidia.com/cuda/cusolver/index.html>`_ library.  
...
.. doxygenstruct:: mpart::TransposeObject
    :members:
    :undoc-members:

.. doxygenfunction:: transpose(Kokkos::View<Traits...> view)

.. doxygenfunction:: mpart::dgemm(double alpha, TransposeObject<MemorySpace> A, TransposeObject<MemorySpace> B, double beta, StridedMatrix<double, MemorySpace> C)
   
.. doxygenfunction:: mpart::AddInPlace(Kokkos::View<double**, Traits1...> x, Kokkos::View<const double**, Traits2...> y)
   
.. doxygenfunction:: mpart::AddInPlace(Kokkos::View<double*, Traits1...> x, Kokkos::View<const double*, Traits2...> y)
   
.. doxygenfunction:: mpart::operator+(Kokkos::View<const ScalarType**, Traits1...> x, Kokkos::View<const ScalarType**, Traits2...> y)
   
.. doxygenfunction:: mpart::operator+(Kokkos::View<const ScalarType*, Traits1...> x, Kokkos::View<const ScalarType*, Traits2...> y)

.. doxygenfunction:: mpart::operator+=(Kokkos::View<ScalarType**, Traits1...> x, Kokkos::View<const ScalarType**, Traits2...> y)
   
.. doxygenfunction:: mpart::operator+=(Kokkos::View<ScalarType*, Traits1...> x, Kokkos::View<const ScalarType*, Traits2...> y)
    
.. doxygenfunction:: mpart::operator*(TransposeObject<MemorySpace> A, TransposeObject<MemorySpace> B)